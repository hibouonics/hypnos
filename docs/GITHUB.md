...
Recommended repository name: `hebuonics-hypnos`.
...
7. Add a short repository description, topics such as `micropython`,
   `raspberry-pi-pico`, `embedded`, `rtc`, and `low-power`, and the Hebuonics
...
```text
Prepare Hebuonics Hypnos firmware for GitHub
```

## Suggested repository description

```text
MicroPython firmware and hardware drivers for the Hebuonics Hypnos low-power controller.
```
